  ...
  return (
    <div className="rp-root">
      ...
      <header className="rp-header">
        <div>
          <h1 className="rp-title">Fish Health Reports</h1>
          <p className="rp-subtitle">
            Real-time fish health intelligence · {lastSync ? `Updated ${lastSync}` : 'Loading…'}
          </p>
        </div>
      ...
      <div className="rp-tabs">
        {[
          ...
          { id: 'records',  label: 'Health Records',    icon: <FileText   size={14}/> },
          { id: 'batches',  label: 'Scan Batches',      icon: <TrendingUp size={14}/> },
          { id: 'audit',    label: 'Activity Log',      icon: <CheckCircle size={14}/> },
        ].map(t => (
          <button key={t.id}
          ...
          {tab === 'overview' && (
            <div className="rp-overview">
              ...
              <div className="rp-kpi-row">
                ...
                <KPICard label="Healthy Rate"
                  value={`${stats?.pass_rate ?? pct(scans.filter(s=>s.status==='PASS').length, scans.length)}%`}
                  sub={`${stats?.pass ?? scans.filter(s=>s.status==='PASS').length} healthy`}
                  accent="#22c55e"/>
                <KPICard label="Infected Rate"
                  value={`${stats?.fail_rate ?? pct(scans.filter(s=>s.status==='FAIL').length, scans.length)}%`}
                  sub={`${stats?.fail ?? scans.filter(s=>s.status==='FAIL').length} infected`}
                  accent="#ef4444"/>
                      ...
                      {batches.filter(b => b.endTime).reverse().map((b, i) => {
                        ...
                        return (
                          <tr key={i} className="rp-tr">
                            ...
                            <td>{b.totalScans}<span className="rp-td-muted"> ({b.defects} infected)</span></td>
                            <td>
