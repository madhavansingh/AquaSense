...
// ── Suggested starters ────────────────────────────────────────────────────
const STARTERS = [
  'What should I do for Aeromoniasis?',
  'How do I improve water quality for sick fish?',
  'What are the signs of fungal infection?',
  'How often should I scan my fish pond?',
  'What is the treatment plan for Parasitic disease?',
];
...
const ChatAssistant = () => {
  ...
  const sendMessage = async (text) => {
    ...
    try {
      ...
      const answer = data.answer || 'Unable to get a response. Please try again.';
      ...
      setMessages(prev => [...prev, { role: 'bot', text: 'Server not responding. Check that the backend is running.', error: true }]);
    } finally {
      setLoading(false);
    }
  };
