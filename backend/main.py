...
from services.bulk_predict_service import (
    ...
    ALLOWED_EXTENSIONS   as _BULK_ALLOWED_EXT,
    MAX_BATCH_SIZE       as _BULK_MAX,
)
...
async def aquaguard_video_endpoint(file: UploadFile = File(...)):
    ...
    if not data:
        raise HTTPException(status_code=400, detail="Empty file uploaded.")
    if len(data) > 100 * 1024 * 1024:      # 100 MB cap for video
        raise HTTPException(status_code=413, detail="Video too large (max 100 MB).")

    loop   = asyncio.get_running_loop()
    result = await loop.run_in_executor(None, _aq_video, data)
    return JSONResponse(content=_safe_json(result))


class AQChatPayload(BaseModel):
    question:    str
...
async def aquaguard_chat_endpoint(payload: AQChatPayload):
    ...
    if not question:
        return {"answer": "Please ask me something about your fish."}

    loop   = asyncio.get_running_loop()
    answer = await loop.run_in_executor(
        None, _aq_chat, question, payload.last_result
    )
    return {"answer": answer}
...
async def aquaguard_bulk_predict(files: List[UploadFile] = File(...)):
          ...
          "results": [
            {
              ...
              "confidence": 0.82,
              "severity": "high",
              "source": "model | filename_hint | hybrid",
              "top_predictions": ["Aeromoniasis", "Parasitic"],
              "filename_hint": "aeromoniasis" | null
            }, ...
          ],
          "summary": {
            "total_images": 5,
            ...
            "uncertain_count": 1,
            "high_risk_count": 2,
            "most_common_disease": "Aeromoniasis",
            "average_confidence": 0.74,
            "source_breakdown": {"model": 4, "filename_hint": 1},
            "disease_distribution": {"Aeromoniasis": 2, "Parasitic": 1}
          },
          "errors": []
        }
    """
    ...
    if len(files) > _BULK_MAX:
        raise HTTPException(
            status_code=400,
            detail=(
                f"Too many files. Maximum allowed is {_BULK_MAX} images per batch."
            ),
        )

    # ── 2. Per-file type + size validation → collect (filename, bytes) ─────
    ...
    for idx, upload in enumerate(files, start=1):
        ...
        if ext not in _BULK_ALLOWED_EXT:
            rejected.append({
                "id"      : fish_id,
                "filename": fname,
                "reason"  : (
                    f"Unsupported file type '{ext}'. "
                    f"Allowed: {', '.join(_BULK_ALLOWED_EXT)}."
                ),
            })
            continue
        ...
        if not data:
            rejected.append({"id": fish_id, "filename": fname, "reason": "Empty file."})
            continue
        ...
        if len(data) > MAX_FILE_BYTES:
            rejected.append({
                "id"      : fish_id,
                "filename": fname,
                "reason"  : f"File too large ({len(data) // 1024} KB). Max 15 MB.",
            })
            continue

        accepted.append((fname, data))

    if not accepted:
        raise HTTPException(
            status_code=422,
            detail={"message": "No valid images could be processed.", "errors": rejected},
        )

    # ── 3. Run batch inference in executor (keeps event loop unblocked) ────
    loop         = asyncio.get_running_loop()
    batch_output = await loop.run_in_executor(None, _aq_bulk_process, accepted)

    response = {
        "results": batch_output.get("results", []),
        "summary": batch_output.get("summary", {}),
        "errors" : rejected + batch_output.get("errors", []),
    }
    return JSONResponse(content=_safe_json(response))
...
class _TreatmentRequest(BaseModel):
    results: list[dict]
...
async def aquaguard_treatment_plan(payload: _TreatmentRequest):
    ...
    if not payload.results:
        raise HTTPException(
            status_code=400,
            detail="'results' list is empty. Pass fish prediction results to generate a plan.",
        )
    if len(payload.results) > 50:
        raise HTTPException(
            status_code=400,
            detail="Too many results. Maximum 50 fish per treatment plan request.",
        )

    loop = asyncio.get_running_loop()
    plan = await loop.run_in_executor(None, _aq_treatment, payload.results)
    return JSONResponse(content=_safe_json(plan))
...
_aq_session_lock   = _threading.Lock()
_aq_session_store: list[dict] = []   # max 10 sessions, newest last
_AQ_MAX_SESSIONS   = 10


class _SessionStorePayload(BaseModel):
    timestamp: str
    results:   list[dict]
    summary:   dict
...
async def aquaguard_session_store(payload: _SessionStorePayload):
    ...
    Body: { "timestamp": "...", "results": [...], "summary": {...} }
    Returns: { "ok": true, "sessions_stored": N }
    """
    entry = {
    ...
    with _aq_session_lock:
        _aq_session_store.append(entry)
        if len(_aq_session_store) > _AQ_MAX_SESSIONS:
            del _aq_session_store[:-_AQ_MAX_SESSIONS]   # keep newest 10

    return {"ok": True, "sessions_stored": len(_aq_session_store)}
...
async def aquaguard_farm_insights():
    ...
    Returns:
    {
      "health_score":      { "score": 0-100, "label": "Good|Moderate|Critical" },
      "risk_level":        { "level": "low|medium|high", "explanation": "..." },
      "trend":             { "trend": "increasing|stable|decreasing", ... },
      "insights":          ["...", "..."],
      "dominant_disease":  "..." | null,
      "prevention_recs":   { "disease": "...", "water_quality": [...], ... },
      "sessions_available": N
    }
    """
    with _aq_session_lock:
        sessions = list(_aq_session_store)

    # Current results = last session's results (most recent scan)
    current_results = sessions[-1]["results"] if sessions else []

    loop    = asyncio.get_running_loop()
    intel   = await loop.run_in_executor(
        None, _aq_farm_insights, current_results, sessions
    )

    intel["sessions_available"] = len(sessions)
    return JSONResponse(content=_safe_json(intel))
